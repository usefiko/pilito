...
class CustomerAdmin(admin.ModelAdmin):
    list_display = ('first_name', 'last_name', 'source', 'has_data', 'created_at', 'id')
    ...
    fieldsets = (
        ('Customer Information', {
            'fields': ('first_name', 'last_name', 'username', 'phone_number', 'email', 'description', 'profile_picture')
        }),
        ('Source & Identity', {
            'fields': ('source', 'source_id', 'tag'),
        }),
        ('Persona & Bio (Instagram)', {
            'fields': ('bio', 'persona_data'),
            'classes': ('collapse',),
            'description': 'Auto-extracted persona info from Instagram bio'
        }),
        ('Custom Data (JSON)', {
            'fields': ('data',),
            'description': 'Custom key-value data in JSON format. Example: {"company": "ACME", "notes": "VIP customer"}'
        }),
        ('Timestamps', {
    ...
    def has_data(self, obj):
        """Show if customer has custom data"""
        if obj.data and len(obj.data) > 0:
            return f"📊 {len(obj.data)} keys"
        return "—"
