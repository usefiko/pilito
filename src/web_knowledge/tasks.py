...
def generate_qa_pairs_task(self, page_id: str, max_pairs: int = 5) -> Dict[str, Any]:
    """
    Async task to generate Q&A pairs for a page using AI
    ...
    try:
        ...
        if not qa_generator.is_available():
            logger.warning("Q&A generator AI not available, using fallback generation")
            # Generate fallback Q&A pairs directly
            qa_pairs_data = _generate_fallback_qa_pairs(page, max(3, max_pairs))
        else:
        ...
        if not qa_pairs_data:
            logger.warning(f"No Q&A pairs generated by AI for page: {page.url}, generating fallback pairs")
            qa_pairs_data = _generate_fallback_qa_pairs(page, max(3, max_pairs))
        
        # Ensure minimum 3 Q&A pairs
        if len(qa_pairs_data) < 3:
            logger.info(f"Ensuring minimum 3 Q&A pairs for page: {page.url}")
            additional_pairs = _generate_fallback_qa_pairs(page, 3 - len(qa_pairs_data))
            qa_pairs_data.extend(additional_pairs)
